...
from .schemas import filters, merge_default, normalize, scopes, security, \
    validators
...
class FlaskValidatorAdaptor(object):
    ...
    def type_convert(self, obj):
        ...
        convert_funs = {
            'integer':
            lambda v: self.validate_number(int, v[0]),
            'boolean':
            lambda v: v[0].lower() not in ['n', 'no', 'false', '', '0'],
            'null':
            lambda v: None,
            'number':
            lambda v: self.validate_number(float, v[0]),
            'string':
            lambda v: v[0]
        }
...
def request_validate(view):
    ...
    def wrapper(*args, **kwargs):
        ...
        # scope
        if (endpoint,
                request.method) in scopes and not set(scopes[(
